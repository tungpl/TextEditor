...
public abstract class Document {
	...
	protected int countSyllables(String word)
	{
		...
		while(i<=cArray.length-1){
			...
			if(isVowel(cArray[i])){	
				...
				if(j<cArray.length&&isVowel(cArray[j])) {
					i=j;
				}
				else{					
					i++;count++;
		...
		if(cArray.length>1&&cArray[cArray.length-1]=='e'&&(!isVowel(cArray[cArray.length-2]))&&count>=2){count--;}
	...
	public double getFleschScore()
	{
		...
		double numSyllabes = this.getNumSyllables();
		fleschScore = 206.835- 1.015*numWords/numSentences - 84.6*numSyllabes/numWords;
	    return fleschScore;
	}
